...
{
	float tr = (seed%16)/16.0f;
	float tg = (seed%8)/8.0f;
	float tb = (seed%4)/4.0f;
	return vec4( 
					0.3 + 0.4 * tr, 
					0.3 + 0.4 * tg, 
					0.3 + 0.4 * tb, 
...
void main()
{
	...
	float factor = (rnd_seed%16)/16.0f;
	vec4 colorR = mix(vec4( 1.0f, 0.89, 0.78, 1.0f ), vec4( 0.18, 0.13, 0.12, 1.0f ), factor);  // Dark skin tone
