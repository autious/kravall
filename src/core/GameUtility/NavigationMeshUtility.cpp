...
namespace Core
{
	void NavigationMesh::DrawDebug()
	{
		...
		{
			float* points = instance->m_nodes[i].points;

			for( int p = 0; p < 4; p++ )
			{
				if( instance->m_nodes[i].corners[p].length < 0 )
					continue;
	...
	bool NavigationMesh::CheckPointInsideNode( glm::vec3 point, int node )
	{
		...
		for( int i = 0; i < 4; i++ )
		{
			if( instance->m_nodes[node].corners[i].linksTo < -1.5f )
				continue;

			int ii = i * 2;
			int oo = (ii + 2) % 8;
			float* points = instance->m_nodes[node].points;
