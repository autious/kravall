#ifndef SRC_CORE_LUA_BRIDGES_COMPONENTBIND_MOVEMENTDATA_COMPONENT_HPP
#define SRC_CORE_LUA_BRIDGES_COMPONENTBIND_MOVEMENTDATA_COMPONENT_HPP

#include <Components/MovementComponent.hpp>
#include <ComponentFramework/SystemTypes.hpp>

#include <Lua/Bridges/LuaEntityBridgeTypes.hpp>

#include <Lua/LuaUtility.hpp>

namespace Core
{
    class MovementDataComponentBinding
    {
    public:
        static ComponentGetters GetGetters();
        static ComponentSetters GetSetters();

        static ComponentType GetComponentType();
        static const char* GetComponentLuaName();
    };
}

#endif
